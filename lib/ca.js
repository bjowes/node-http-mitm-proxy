...
CA.prototype.generateServerCertificateKeys = function (hosts, cb) {
  ...
  FS.writeFile(path.join(this.certsFolder, this.hostToFilename(mainHost, '.pem')), certPem, function (error) {
    if (error) console.error('Failed to save certificate to disk in ' + self.certsFolder, error);
  });
  FS.writeFile(path.join(this.keysFolder, this.hostToFilename(mainHost, '.key')), keyPrivatePem, function (error) {
    if (error) console.error('Failed to save private key to disk in ' + self.keysFolder, error);
  });
  FS.writeFile(path.join(this.keysFolder, this.hostToFilename(mainHost, '.public.key')), keyPublicPem, function (error) {
