...
Proxy.prototype._onHttpServerConnectData = function (req, socket, head) {
  ...
  function makeConnection(port, host) {
    ...
    conn.on("finish", () => connClosing(socket));
    conn.on("close", () => connClosing(socket));
    socket.on("end", () => {
      socketClosing(conn);
      connClosing(socket);
    });
    socket.on("finish", () => socketClosing(conn));
    socket.on("close", () => socketClosing(conn));
  }
          ...
          httpsOptions: [
            ...
            function (data, callback) {
              if (data.keyFileExists && data.certFileExists) {
                ...
                return self.onCertificateMissing(
                  ctx,
                  files,
                  function (err, files) {
                    if (err) {
          ...
          if (self.forceSNI && !hostname.match(/^[\d\.]+$/)) {
            ...
            try {
              self._createHttpsServer(
                results.httpsOptions,
                function (port, httpsServer, wssServer) {
...
Proxy.prototype.onCertificateMissing = function (ctx, files, callback) {
  ...
  this.ca.generateServerCertificateKeys(
    hosts,
    function (certPEM, privateKeyPEM) {
      callback(null, {
...
Proxy.prototype._onError = function (kind, ctx, err) {
  ...
  if (ctx) {
    ...
    if (
      ...
      ctx.proxyToClientResponse.writable &&
      !ctx.proxyToClientResponse.headersSent
    ) {
...
Proxy.prototype._onHttpServerRequest = function (
  isSSL,
...
Proxy.prototype._onWebSocketFrame = function (
  ctx,
  type,
  fromServer,
  data,
...
Proxy.prototype._onWebSocketClose = function (
  ctx,
  closedByServer,
  code,
  message
